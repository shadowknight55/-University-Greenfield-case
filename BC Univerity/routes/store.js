...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const student = await Student.findByPk(studentId); // Fetch the student's balance
    const balance = student ? parseFloat(student.balance) : 0; // Ensure balance is a number or default to 0
    res.render("store", { items, balance }); // Render the store page with the items and balance
  } catch (error) {
...
router.post("/purchase", async (req, res) => {
  ...
  try {
    ...
    if (student && item) {
      if (item.quantity > 0) {
        const newBalance = student.balance - parseFloat(itemPrice);
        if (newBalance < 0) {
